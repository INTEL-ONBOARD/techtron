...
const DateRangePicker = ({
  ...
  const renderCalendar = () => {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      let className =
        "flex items-center justify-center cursor-pointer w-[46px] h-[46px] rounded-full text-gray-800 hover:bg-[#1A318C] hover:text-white";

      if (selectedStartDate && dayString === selectedStartDate) {
        className += " bg-[#1A318C] text-white rounded-r-none";
      }
      if (selectedEndDate && dayString === selectedEndDate) {
        className += " bg-[#1A318C] text-[#ffffff] rounded-l-none";
      }
      if (
        selectedStartDate &&
        selectedEndDate &&
        ...
        className += " bg-[#1A318C] text-white rounded-none";
      }
  ...
  return (
    <div className="relative" ref={datepickerRef}>
                <div className="relative flex items-center">
                  <span className="absolute left-0 pl-5 text-gray-800">
                    <svg
                ...
                className="w-full rounded-lg text-sm p-6 bg-[#F8F8F8] border border-[#EBEBEB] py-2.5 pl-[50px] pr-8 placeholder-[#D3D3D3] text-gray-500 outline-none transition focus:ring-blue-400 focus:border-[#1A318C] "
                value={updateInput()}
        ...
        <span
          className="absolute right-0 cursor-pointer pr-4 text-gray-800"
          onClick={toggleDatepicker}
        >
          <svg
      ...
      {isOpen && (
        ...
        >
          <div className="flex items-center justify-between px-5">
            <button
              id="prevMonth"
              className="rounded-md px-2 py-2 text-gray-800 hover:bg-gray-2 "
              onClick={() =>
                setCurrentDate(
                  new Date(
          ...
          <div className="mt-5 flex justify-end space-x-2.5 border-t border-stroke p-5 ">
            <button
              id="cancelButton"
              className="rounded-lg border border-[#1A318C] px-5 py-2.5 text-base font-medium text-[#1A318C] hover:bg-blue-light-5"
            ...
            <button
              id="applyButton"
              className="rounded-lg bg-[#1A318C] px-5 py-2.5 text-white font-medium hover:text-white hover:bg-[#1B44C8]"
              onClick={handleApply}
            >
              Apply
            </button>
